...
Advance your JavaScript skills with free access to 5 Frontend Masters courses! No card required.
Mozilla ads
...
const result = text
    ...
    .filter(isValidChar) // Filter out all invalid characters
    .map(lowercase)
    .reduce(charCounter, {}); // Map each character to lowercase
...
const sortedValues = Object.entries(result)
    .sort(sortByValue) // Sort the entries by value
    .reverse() // Reverse the array
    .filter((entry, index) => index < 10); // Only show the first 10 entries
